...
class MockCocktailSearchDataSource extends Mock implements CocktailSearchRemoteRetroFitDataSource {}
...
void main() {
  ...
  setUp(() {
    ...
    sut = CocktailSearchRepository(cocktailSearchRemoteRetroFitDataSource: dataSource);
  });

  group('getCocktailModelByName', () {
    test('should call remoteDataSource.getCocktailDataByName(cocktailName) method', () async {
      // 1
      when(() => dataSource.getCocktailDataByName('mojito')).thenAnswer(
        (_) async => const CocktailsDto(
            drinks: [CocktailDto(strDrink: 'mojito', strInstructions: '', strDrinkThumb: 'imageURL')]),
      );
      // 2
    ...
    test('should filter data source results with provided cocktailName', () async {
      // 1
      when(() => dataSource.getCocktailDataByName('mojito')).thenAnswer(
        (_) async => const CocktailsDto(
            drinks: [CocktailDto(strDrink: 'mojito', strInstructions: '', strDrinkThumb: 'imageURL')]),
